...
class RushHour:
    ...
    def solution(niveau, return_graph=False, verbeux=False):
        ...
        plateau = Plateau(niveau)
        prev = {plateau: None}
        todo = deque([(plateau, 0)])
        if plateau.est_gagnant():
            return []
        while todo:
            # avec pop(): parcours en profondeur, avec popLeft(): en largeur
            sommet, dist = todo.pop()
            for deplacement, voisin in (tqdm(sommet.voisins().items(),
                                             f"Visiting neighbors at distance {dist}")
                                        if verbeux else sommet.voisins().items()):
                if voisin not in prev:
                    prev[voisin] = deplacement, sommet
                    todo.append((voisin, dist+1))
                    if voisin.est_gagnant():
                        sommet = voisin
                        path = []
                        while prev[sommet] is not None:
                            deplacement, sommet = prev[sommet]
                            path.append(deplacement)
                        if return_graph:
                            return path[::-1], prev
                        else:
                            return path[::-1]
