...
def default_layout(self: networkx.Graph) -> Dict:
    try:
        ...
        with warnings.catch_warnings():
            ...
            return networkx.planar_layout(self)
    except networkx.NetworkXException:
        return networkx.spring_layout(self)
...
def show(self: networkx.Graph) -> Any:
    ...
    edge_labels = {(u, v): w
                   for (u, v), w in edge_labels.items() if w is not None and w != 1}
    layout = default_layout(self)

    plt.figure()
    networkx.draw(self, layout, width=3, linewidths=1,
                  node_size=500, node_color='lightgray', alpha=0.9,
                  labels={node : node for node in self.nodes()})
    networkx.draw_networkx_edge_labels(self, layout,
